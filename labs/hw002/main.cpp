...
void cmd_state(const GameState& state) {
    ...
    std::cout << "state pos=" << state.piece_position.x << "," << state.piece_position.y
              << " run=" << btext(state.running)
              << " shape=" << shape.size() << "x" << (shape.empty() ? 0u : shape.front().size())
              << " cells=";
...
int main() {
    ...
    while (input >> token) {
        if (token == "move") {
            ...
            int y = 0;
            input >> x >> y;
            std::cout << "inb(" << x << "," << y << ")=" << btext(state.board.inBounds({x, y})) << '\n';
        } else if (token == "state") {
            cmd_state(state);
        } else if (token == "reset") {
            init_game(state);
